...
sjcl.mode.ccm = {
   ...
   */
  encrypt: function(prf, plaintext, iv, adata, tlen) { //O_
   ...
   */
  decrypt: function (prf, ciphertext, iv, adata, tlen) { //O_
    tlen = tlen || 64;
    adata = adata || [];
    var L,
        w=sjcl.bitArray,
 ...
 */
 ...
 sjcl.json = {
  ...
  defaults: { v:1, iter:1000, ks:256, ts:128, mode:"ccm", adata:"", cipher:"aes" },
...
sjcl.arrayBuffer.ccm = {
  ...
  encrypt: function(prf, plaintext_buffer, iv, adata, tlen, ol){
    ...
    var padded_length = plaintext_buffer.byteLength + tlen;
    var padded_array = new Uint8Array(padded_length);
    padded_array.set(new Uint8Array(plaintext_buffer));
    var padded_buffer = padded_array.buffer;
    ...
    mac = sjcl.arrayBuffer.ccm._computeTag(prf, padded_buffer, iv, adata, tlen, ol, L);

    //encrypt the plaintext and the mac 
    //returns the mac since the plaintext will be left encrypted inside the buffer
    mac = sjcl.arrayBuffer.ccm._ctrMode(prf, padded_buffer, iv, mac, tlen, ol, L);
    mac = sjcl.codec.arrayBuffer.fromBits(mac, false);

    padded_array.set(new Uint8Array(mac), ol);
   ...
   */
  decrypt: function(prf, ciphertext_buffer, iv, tag, adata, tlen){
    var mac, mac2, i, L, w = sjcl.bitArray, 
      ivl = w.bitLength(iv) / 8, data;
    ...
    var ol = ciphertext_buffer.byteLength - tlen;

    var tag = sjcl.codec.arrayBuffer.toBits(ciphertext_buffer.slice(ol));

    data = new DataView(ciphertext_buffer);
    for (i = ol; i < ciphertext_buffer.byteLength; i++) {
        data.setUint8(i, 0);
    }
    ...
    //decrypt the buffer
    mac = sjcl.arrayBuffer.ccm._ctrMode(prf, ciphertext_buffer, iv, tag, tlen, ol, L);
   ...
   * @private
   */
  _ctrMode: function(prf, data_buffer, iv, mac, tlen, ol, L){
    ...
    if (data_buffer.byteLength !== 0) {
      ...
      for (i=0; i<ol;i+=16){
        if (i > n) {
          sjcl.mode.ccm._callProgressListener(i/ol);
          n += p;
        }
